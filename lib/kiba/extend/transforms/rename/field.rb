...
module Kiba::Extend::Transforms::Rename
  # Renames one field
  #
  # ## Example notes
  # ### 1 - :from field exists
  ...
  #
  # :sex is renamed to :gender, overwriting the existing value of :gender
  #   (unknown) with value of :sex (m). The transform emits a warning about
  #   this
  #
  # ### 3 - :from field does not exist
  #
  # Row is passed through unchanged, and the transform emits a warning about
  #   not being able to rename a field that doesn't exist.
  #
  # ### 4 - :from and :to field are the same
  #
  # This seems like a real weird edge case from the perspective of defining
  #   transforms manually, but it can happen when transform definitions are
  #   programmatically generated from configuration.
  #
  ...
  class Field
    ...
    def process(row)
      ...
      unless row.key?(from)
        add_single_warning("Cannot rename field: `#{from}` does not exist "\
                           "in row")
        return row
      end

      if row.key?(to)
        add_single_warning("Renaming `#{from}` to `#{to}` overwrites existing "\
                           "`#{to}` field data")
      end
