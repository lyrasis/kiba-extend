        ...
        class Fields
          ...
          def process(row)
            ...
            targetvals = if @multival
                           targetvals.map { |val| val.split(@sep, -1).map { |e| e.strip } }
                         else
            ...
            targetvals = if @multival
                           targetvals.map { |vals| vals.join(@sep) unless vals.nil? }
                         else
                           targetvals.map { |vals| vals.first }
                         end
            ...
            unless fv.nil?
              ...
              if delete.size > 0
                ...
                h.each do |f, val|
                  ...
                  row[f] = val.size > 0 ? val.join(@sep) : nil
                end
              end
            end
