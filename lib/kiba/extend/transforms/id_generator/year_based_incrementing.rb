...
module Kiba
  module Extend
    module Transforms
      module IdGenerator
        # Generates unique ID values based on the year extracted from a given
        #   date field, where available. For each year extracted, the
        #   rows are sorted by the full date value, and the
        #   id number value is incremented.
        #
        # The transform supports a multi-segment number pattern. All segments
        #   except for the year and incrementing digit(s) are optional. Set them
        #   as empty Strings when you set up the transform to omit them. The
        ...
        #
        # - You will have already reduced the output to one row per unique ID
        ...
        #   ISO 8601 dates, that will be set as the `datefield` param
        #
        ...
        class YearBasedIncrementing
          ...
          # @return [Array{Hash}] Array of rows to be yielded by `close` method
          #   of using class
          def finalized_rows
          ...
          attr_reader :datefield, :target, :prefix, :preincrementsegment,
            :nodateval, :sorter, :holder
          ...
          def generate_for_year(year, data)
            base = "#{prefix}#{year}#{preincrementsegment}"
            counter = 0
