...
RSpec.describe Kiba::Extend::Data::ConvertibleFraction do
  ...
  describe '#replace_in' do
    ...
    let(:results) do
      expectations.keys.map{ |arr| klass.new(**arr[0]).replace_in(val: arr[1]) }
    end
  ...
  describe '#==' do
    ...
    let(:results) do
      expectations.keys.map{ |pair| klass.new(**pair[0]) == klass.new(**pair[1]) }
    end
    let(:expected){ expectations.values }
  ...
  describe '#hash' do
    ...
    let(:results) do
      expectations.keys.map{ |pair| klass.new(**pair[0]).hash == klass.new(**pair[1]).hash }
    end
    let(:expected){ expectations.values }
