RSpec.describe Kiba::Extend::Transforms::Helpers::FieldValueGetter do
  ...
  describe '#call' do
    ...
    context 'with treat_as_null = %NULLVALUE%' do
      let(:params){ {fields: fields, delim: delim, treat_as_null: '%NULLVALUE%'} }
      let(:expected) do
        {
          d: 'foo',
          g: '%NULL%|%NULLVALUE%'
    ...
    context 'with treat_as_null = [%NULL%, %NULLVALUE%]' do
      let(:params){ {fields: fields, delim: delim, treat_as_null: ['%NULL%', '%NULLVALUE%']} }
      let(:expected) do
        {
          d: 'foo'
        }
    ...
    context 'with discard = [] and treat_as_null = [%NULL%, %NULLVALUE%]' do
      let(:params){ {fields: fields, delim: delim, discard: [], treat_as_null: ['%NULL%', '%NULLVALUE%']} }
      let(:expected) do
      {
        a: nil,
        b: '',
    ...
    context 'with discard = [:nil] and treat_as_null = [%NULL%, %NULLVALUE%]' do
      let(:params){ {fields: fields, delim: delim, discard: [:nil], treat_as_null: ['%NULL%', '%NULLVALUE%']} }
      let(:expected) do
        {
          b: '',
          c: '|',
    ...
    context 'with discard = [:empty] and treat_as_null = [%NULL%, %NULLVALUE%]' do
      let(:params){ {fields: fields, delim: delim, discard: [:empty], treat_as_null: ['%NULL%', '%NULLVALUE%']} }
      let(:expected) do
        {
          a: nil,
          c: '|',
    ...
    context 'with discard = [:delim] and treat_as_null = [%NULL%, %NULLVALUE%]' do
      let(:params){ {fields: fields, delim: delim, discard: [:delim], treat_as_null: ['%NULL%', '%NULLVALUE%']} }
      let(:expected) do
        {
          a: nil,
          b: '',
