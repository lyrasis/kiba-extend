...
RSpec.describe Kiba::Extend::Transforms::FilterRows::FieldMatchRegexp do
  ...
  let(:transform){ described_class.new(action: action, field: field, match: match) }
  ...
  context 'with action: :keep' do
    ...
    context 'with case insensitive flag' do
      let(:transform){ described_class.new(action: action, field: field, match: match, ignore_case: true) }
      let(:expected) do
        [
          {val: 'N'},
          {val: 'n'},
