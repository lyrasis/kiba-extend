...
RSpec.describe Kiba::Extend::Transforms::Merge::ConstantValueConditional do
  subject(:merger){ described_class.new(fieldmap: fieldmap, condition: condition) }

  let(:fieldmap){ { reason: 'gift', cost: '0' } }
  let(:condition){ ->(row){ row[:note].is_a?(String) && row[:note].match?(/gift|donation/i) && row[:type] != 'obj' } }
  let(:input) do
    [
      {note: 'Gift', type: 'acq'},
