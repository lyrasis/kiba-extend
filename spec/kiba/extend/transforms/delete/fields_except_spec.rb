...
RSpec.describe Kiba::Extend::Transforms::Delete::Fields do
  ...
  context 'with keepfields given' do
    ...
    it 'puts warning to STDOUT' do
      msg = %Q[#{Kiba::Extend.warning_label}: The `keepfields` keyword is being deprecated in a future version. Change it to `fields` in your ETL code.\n]
  ...
  context 'with fields and keepfields given' do
    ...
    it 'puts warning to STDOUT' do
      msg = %Q[#{Kiba::Extend.warning_label}: Do not use both `keepfields` and `fields`. Defaulting to process using `fields`\n]
      expect{ result }.to output(msg).to_stdout
    end
  end
  ...
  context 'with neither fields and keepfields given' do
    ...
    it 'puts raises MissingKeywordArgumentError' do
      expect{ result }.to raise_error(Delete::FieldsExcept::MissingKeywordArgumentError)
    end
  end
end
