...
RSpec.describe Kiba::Extend::Transforms::Delete::FieldValueIfEqualsOtherField do
  ...
  context 'with grouped field(s)' do
    context 'where groups are even as expected' do
      let(:input) do
        [
          {row: '1', del: 'A;C;d;c;e', compare: 'c', grpa: 'y;x;w;u;v', grpb: 'e;f;g;h;i'},
          {row: '2', del: 'a;b;c', compare: 'a;z;c', grpa: 'd;e;f', grpb: 'g;h;i' },
    ...
    context 'where groups are ragged' do
      ...
      it 'outputs warning to STDOUT' do
        msg = Regexp.new(
          /KIBA WARNING: One or more grouped fields \(grpa, grpb\) has different number of values than the others in \{.*\}/
        )
