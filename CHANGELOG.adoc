...
* **Only relevant if you have called global `Kiba::Extend::DELIM` from outside Kiba::Extend for some reason.** This global constant was finally removed from the final place it was being used within kiba-extend, and it has been removed from the application setup.

==== Added

* New service object classes in `Transforms::Helpers`: `DelimOnlyChecker`, `FieldValueGetter`, `RowFieldEvennessChecker`, `ValhashFieldEvennessChecker`, and `ValhashFieldEvennessFixer`. In https://github.com/lyrasis/kiba-extend/pull/92[PR#92]
* New transforms: `Warn::UnevenFields`, `Clean::EvenFieldValues`, `Delete::DelimiterOnlyFieldValues`, `Delete::EmptyFieldGroups`, `Deduplicate::FlagAll`, `Collapse::FieldsWithCustomFieldmap`, `Collapse::FieldsToRepeatableFieldGroup`, and `Collapse::FieldsToTypedFieldPair`. In https://github.com/lyrasis/kiba-extend/pull/92[PR#92]
