...
* Delete `name` param from `NullJob`
* Delete https://lyrasis.github.io/kiba-extend/Kiba/Extend/Utils/MultiSourceNormalizer.html[`Utils::MultiSourceNormalizer`]
* Delete https://lyrasis.github.io/kiba-extend/Kiba/Extend/Jobs/MultiSourcePrepJob.html[`Jobs::MultiSourcePrepJob`]
...
*  `Kiba::Extend::Job.output_fields` returns empty Array instead of `nil` when there is no job output.
The reason for this change is that it good practice to return values with the same method signatures so that code using this method doesn't always have to account for the type of returned value.
Code that does specific things based on "nil-ness" of this value will no longer work as expected, so, for now, a warning is emitted when an empty array is returned.
Code should not start throwing exceptions because of this change, since code expecting an array of field values will not crash when receiving an empty array.
https://github.com/lyrasis/kiba-extend/pull/291[PR#291]
* `Kiba::Extend.warning_label` now includes colon suffix.
Does not affect code functionality, but may produce visual annoyance.
This should be easily remedied with global project find/replace.
https://github.com/lyrasis/kiba-extend/pull/291[PR#291]

=== Added

* `DynamicSource` registry entry type
https://github.com/lyrasis/kiba-extend/pull/274[PR#274]
* `Kiba::Extend::Jobs::DynamicJobable` mixin for project job classes that don't rely on Kiba transforms to manipulate data.
https://github.com/lyrasis/kiba-extend/pull/274[PR#274]
* Add `ProjectConfig.mermaid_config_path` setting that, if set, will be passed on calls to create a mermaid graph for a job.
https://github.com/lyrasis/kiba-extend/pull/274[PR#274]
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Transforms/DateValue/ForceDayPrecision.html[`DateValue::ForceDayPrecision` transform]
https://github.com/lyrasis/kiba-extend/pull/274[PR#275]
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Transforms/Cspace/Movementreferencenumber.html[`Cspace::Movementreferencenumber` transform]
https://github.com/lyrasis/kiba-extend/pull/274[PR#276], https://github.com/lyrasis/kiba-extend/pull/278[PR#278]
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Transforms/IdGenerator/YearBasedIncrementing.html[`IdGenerator::YearBasedIncrementing` transform]
https://github.com/lyrasis/kiba-extend/pull/278[PR#278]
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Transforms/Cspace/Valuationcontrolrefnumber.html[`Cspace::Valuationcontrolrefnumber` transform]
https://github.com/lyrasis/kiba-extend/pull/278[PR#278]
* `XmlDir` https://lyrasis.github.io/kiba-extend/Kiba/Extend/Sources/XmlDir.html[Source] and https://lyrasis.github.io/kiba-extend/Kiba/Extend/Destinations/XmlDir.html[Destination], and https://lyrasis.github.io/kiba-extend/Kiba/Extend/Jobs/XmlJob.html[`XmlJob`]
https://github.com/lyrasis/kiba-extend/pull/288[PR#288]
* Add warning suppression params to `Rename::Field` and `Rename::Fields` transforms
https://github.com/lyrasis/kiba-extend/pull/290[PR#290]
* *Support for handling blank jobs (i.e. jobs that emit no output).*
This includes a new https://lyrasis.github.io/kiba-extend/Kiba/Extend/ProjectConfig#blank_jobs-class_method[`Kiba::Extend::ProjectConfig.blank_jobs`] setting.
https://lyrasis.github.io/kiba-extend/Kiba/Extend/Job#output%3F-class_method[`Kiba::Extend::Job.output?`] pushes jobs with no output to this setting.
Client projects can also push jobs to the setting.
https://lyrasis.github.io/kiba-extend/Kiba/Extend/Job#output%3F-class_method[`Kiba::Extend::Job.output?`] and https://lyrasis.github.io/kiba-extend/Kiba/Extend/Job#output_fields-class_method[`Kiba::Extend::Job.output_fields`] both check the `blank_jobs` value first and return accordingly
https://github.com/lyrasis/kiba-extend/pull/291[PR#291]
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Warnable#warn-instance_method[`Kiba::Extend.warn`] method
https://github.com/lyrasis/kiba-extend/pull/291[PR#291]
...
* Extract no-initial data conversion job logic to https://lyrasis.github.io/kiba-extend/Kiba/Extend/Jobs/AbstractNoInitialDataConversionJob.html[`AbstractNoInitialDataConversionJob`], subclassing https://lyrasis.github.io/kiba-extend/Kiba/Extend/Jobs/MarcJob.html[`MarcJob`] and https://lyrasis.github.io/kiba-extend/Kiba/Extend/Jobs/XmlJob.html[`XmlJob`] to this for clarity when used in projects
https://github.com/lyrasis/kiba-extend/pull/288[PR#288]
* Add two `MarcJob` tests demonstrating the limitations of interaction between Job classes and Sources/Transforms/Destinations
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Registry/Creator[Kiba::Extend::Registry::Creator]'s private `setup_module_spec` method now recognizes job methods that are mixed into job definition modules
https://github.com/lyrasis/kiba-extend/pull/291[PR#291]
...
* CSV Destination now forces quotes around all output values.
https://github.com/lyrasis/kiba-extend/pull/252[PR#252]
* Improvements to `Kiba::Extend::Utils::StringNormalizer` changed this class' API
https://github.com/lyrasis/kiba-extend/pull/252[PR#252]
* Minor changes to API of `Deduplicate::FieldGroup` and `Deduplicate::GroupedFieldValues` transforms: `normalized` parameter was changed to `normalize`.
https://github.com/lyrasis/kiba-extend/pull/252[PR#252]
* Removal of `multival` parameter from `Delete::FieldValueIfEqualsOtherField`
* Removal of `sep` and `multival` parameters from `Deduplicate::Fields`
* Removal of `sep` & `multival` parameters from `Clean::RegexpFindReplaceFieldVals`
* Removal of `multival` parameter from `Prepend::ToFieldValue`
* Removal of `sep` & `multival` parameters from `Explode::FieldValuesToNewRows`
* Removal of `sep` & `multival` parameters from `Replace::FieldValueWithStaticMapping`
...
https://github.com/lyrasis/kiba-extend/pull/252[PR#252]
* Documentation and reusable code for a general value splitting/review/correction FCAR process
...
* `thor project empty_field_report` command to generate a CSV report of all empty fields in files at the paths specified by the registry entries selected by the given tags
https://github.com/lyrasis/kiba-extend/pull/266[PR#266]
* https://lyrasis.github.io/kiba-extend/Kiba/Extend/Utils/TestHelpers.html[`Utils::TestHelpers` module]
https://github.com/lyrasis/kiba-extend/pull/267[PR#267]
* `Kiba::Extend.reset_registry` method. For examples of when this is useful, see https://lyrasis.github.io/kiba-extend/file.common_patterns_tips_tricks.html#manipulate-registry[Common patterns, tips, and tricks > Manipulating the registry on the fly]
https://github.com/lyrasis/kiba-extend/pull/269[PR#269]
* `Kiba::Extend::Jobs::NullJob` for use in dynamically defined projects
https://github.com/lyrasis/kiba-extend/pull/272[PR#272]

==== Bugfixes

* `FileRegistryEntry` summary display issues described in https://github.com/lyrasis/kiba-extend/issues/125[#125]
https://github.com/lyrasis/kiba-extend/pull/263[PR#263]
...
In https://github.com/lyrasis/kiba-extend/pull/242[PR#242] with minor fix added in https://github.com/lyrasis/kiba-extend/pull/243[PR#243]
...
* Use Ruby 4.0.1 and Bundler 4.0.6
