...
- **dynamic source entries**: These entries represent documentation/description of data sources for dynamically defined jobs, when the sources cannot be otherwise represented in kiba-extend. An example might be an "extract all name values" job that runs over all files in the `orig` directory and writes out one destination file. The point of dynamically defining this job is to avoid having to manually list every orig file as a source, and, often, to implment "plain Ruby" manipulation of data (often involving system command calls to utilities like `sed`) in a much more performant (and low effort) way than would be required within the constraints of a normal kiba-extend job. Dynamic source entries serve a couple of purposes: (1) give you a searchable key that enables you to find all dynamic jobs using the same source; and (2) output something meaningful to STDOUT when such a job is running, AND to any job graphs. Dynamic source entries are indicated by including `dynamic_source: true` in the registry entry hash. Ideally, also include a description of the sources involved in `desc`. Since dynamic source entries typically represent multiple files, they cannot be used as lookups or destinations.
